...
public class OSGiWebAppContext extends WebAppContext {
  private static final Log LOG = LogFactory.getLog(OSGiWebAppContext.class);
   ...
   * 
   */
  public OSGiWebAppContext(ClassLoader containerClassLoader) throws CommonObjectConfigurationException, IOException {
      ...
      synchronized (lock) {
        if (webappClassLoader == null) {
          try {
            ...
            parentClassloader = new SwitchedClassLoader(new URL[]{}, commonObject.getOSGiClassLoader(), containerClassLoader);
