...
public class UserEnvironmentListener implements JcrContentListener {
	 ...
	 */
	public void onEvent(int type, String userID, String filePath,
			String fileName) {
			...
			if (fileName.equals(UserFactoryService.USERENV)) {
				...
				try {
					...
					if (userEnvBody != null & userEnvBody.length() > 0) {
						UserEnvironmentBean ue = beanConverter.convertToObject(
								userEnvBody, UserEnvironmentBean.class);
						ue.seal();

						userEnvironmentResolverService.expire(ue.getUser()
								.getUuid());
						...
						query.setParameter(GroupMembershipBean.USER_PARAM, ue
								.getUser().getUuid());
						...
						for (Object o : membershipList) {
							...
							String subjectToken = groupMembershipBean
									.getSubjectToken();
							boolean found = false;
						...
						for (String subject : ue.getSubjects()) {
							...
							if (!found) {
								toAdd.add(new GroupMembershipBean(ue.getUser()
										.getUuid(), subject));
							}
						}
					...
					LOG.warn("Failed to read userenv for " + filePath
							+ " cause :" + e.getMessage());
					LOG.debug(e);
